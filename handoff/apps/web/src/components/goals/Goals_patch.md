# Metas — patch + simulador FIRE (novo, client-side, sem backend)

## 1. Lista de metas — restilizar `GoalsClient.tsx` (ícone quadrado + barra)

Trocar o item de meta por:

```tsx
<div className="cursor-pointer" style={{ opacity: 1 }}>
  <div className="flex items-center gap-2.5">
    <div className="w-[30px] h-[30px] rounded-[9px] flex items-center justify-center text-[13px] flex-shrink-0" style={{ background: goal.bg ?? "var(--surface-2)", color: goal.color ?? "var(--accent)" }}>
      {goal.icon ?? "◎"}
    </div>
    <div className="flex-1 min-w-0">
      <div className="text-[12.5px] font-medium truncate text-[var(--text-primary)]">{goal.name}</div>
      <div className="text-[10.5px] text-[var(--text-muted)]">{etaLabel(goal)}</div>
    </div>
  ...
  <div className="h-[5px] rounded-full overflow-hidden mt-2" style={{ background: "var(--border)" }}>
    <div className="h-full rounded-full" style={{ width: `${Math.min(100, Number(goal.pct_complete) * 100)}%`, background: goal.color ?? "var(--accent)" }} />
  </div>
</div>
```

`etaLabel`: se a API não devolver ETA calculada, computar client-side com o mesmo aporte
mensal médio do goal (`(target - current) / avg_monthly_contribution` em meses) — checar se
`GoalsClient.tsx` já expõe isso antes de duplicar a conta.

## 2. Simulador de independência financeira — feature nova (não existe no repo)

Não há endpoint pra isto; é cálculo puro (regra dos 4% + juros compostos), então não precisa
de backend — dá pra construir como componente client-side alimentado pelo patrimônio real.

```tsx
// apps/web/src/components/goals/FireSimulator.tsx
"use client";
...
export function FireSimulator({ currentInvested, currentAge }: { currentInvested: number; currentAge?: number }) {
  ...
  // barra de acumulação: 20 pontos ao longo dos anos até o alvo
    ...
    <div className="rounded-[18px] p-5.5" style={{ border: "1px solid var(--border)", background: "linear-gradient(180deg,color-mix(in srgb,var(--accent) 7%,transparent),var(--t1))" }}>
      ...
      <div className="flex items-baseline gap-3 mt-4.5 flex-wrap">
        <div>
          ...
          <div className="text-[11.5px] text-[var(--text-secondary)] mt-1.5">
            até a liberdade{currentAge != null ? ` · você terá ${currentAge + wholeYears} anos` : ""}
          </div>
        </div>
...
Integração em `GoalsClient.tsx`: `currentInvested` = `summary.total_market_value_brl` (já
disponível via `getPortfolioSummary`/`getConsolidatedSummary`, mesmo dado da Visão Geral —
não criar query nova). `currentAge` é opcional; se o perfil do usuário não guarda idade, omitir
(não inventar).
