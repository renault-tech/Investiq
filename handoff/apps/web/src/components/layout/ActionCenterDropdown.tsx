 ...
 * impacto (o backend já ordena; aqui só renderiza). Complementa, não
 * substitui, NotificationsDropdown: notificações são eventos que já
 * aconteceram (alerta disparou, orçamento passou); isto é "o que falta
 * você fazer" — foco em ação, não em aviso. */
export function ActionCenterDropdown() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[340px] bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-lg z-50 max-h-[420px] flex flex-col">
          <div className="flex items-center justify-between p-3 border-b border-[var(--border)]">
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">Central de ações</h3>
            {data && data.total_amount > 0 && (
              <span className="text-xs font-mono text-[var(--danger)]">{mask(formatBRLCompact(data.total_amount))} pendente</span>
            )}
          </div>
                  ...
                  return (
                    <li key={item.id}>
                      ...
                      >
                        ...
                        {item.amount !== null && (
                          <span className="text-xs font-mono text-[var(--text-secondary)] shrink-0">{mask(formatBRLCompact(item.amount))}</span>
                        )}
                      </Link>
                    </li>
                  );
...
// Integração em TopBar.tsx: importar e renderizar ao lado de
// <NotificationsDropdown /> (mesmo grupo de ícones à direita da topbar).
