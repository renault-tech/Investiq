...
import { AlertTriangle, Copy, Receipt, TrendingUp } from "lucide-react";
...
export function InvoiceAnalytics({ invoiceId }: InvoiceAnalyticsProps) {
  ...
  return (
    // responsive-grid-12: sem essa classe o gridTemplateColumns inline vence
    // qualquer breakpoint do Tailwind e força rolagem horizontal no celular
    // (ver a regra com !important em globals.css).
    <div className="responsive-grid-12 grid gap-[18px]" style={{ gridTemplateColumns: "repeat(12,1fr)" }}>
                ...
                <span className="flex items-baseline gap-2">
                  ...
                  {c.delta_pct !== null && (
                    <span
                      ...
                      title="Variação sobre a média das faturas anteriores"
                    >
                      {c.delta_pct > 0 ? "+" : ""}
      ...
      <section className="col-span-6 border border-[var(--border)] bg-[var(--surface)] rounded-[var(--radius-card-sm)] p-5">
        <div className="flex items-baseline justify-between mb-4">
          ...
          <span className="font-mono text-[11px] text-[var(--text-muted)]">
            média {mask(formatBRLCompact(data.avg_amount))}
          </span>
        </div>
            ...
            <div key={t.reference_month} className="flex-1 flex flex-col items-center gap-1.5">
              <div className="w-full h-[76px] flex items-end">
                <div
                  ...
                  title={`${monthShort(t.reference_month)}: ${mask(formatBRL(t.total_amount))}`}
                  style={{
      ...
      {data.top_items.length > 0 && (
        <section className="col-span-12 border border-[var(--border)] bg-[var(--surface)] rounded-[var(--radius-card-sm)] p-5">
          <h4 className="text-sm font-semibold text-[var(--text-primary)] mb-3">Maiores gastos da fatura</h4>
            ...
            {data.top_items.map((item) => (
              <li key={item.id} className="flex items-baseline justify-between gap-3 text-[12px]">
                ...
                <span className="font-mono text-[var(--text-primary)] whitespace-nowrap">
                  {mask(formatBRL(Number(item.amount)))}
                </span>
              </li>
            ))}
          </ul>
      ...
      {data.findings.length > 0 && (
        <section className="col-span-12 border border-[var(--border)] bg-[var(--surface)] rounded-[var(--radius-card-sm)] overflow-hidden">
          ...
          {data.findings.map((f, i) => {
            const Icon = FINDING_ICON[f.kind] ?? Receipt;
            return (
              <div key={i} className="flex items-start gap-3 px-5 py-3 border-t border-[var(--border)]">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    ...
                    {f.amount !== null && (
                      <span className="font-mono text-[11px] text-[var(--text-secondary)]">{mask(formatBRL(f.amount))}</span>
                    )}
                  </div>
